...
let arrayGraduadosHTML5 = graduadosHTML5.split(" ").map(Number);
let arrayGraduadosCSS3 = graduadosCSS3.split(" ").map(Number);
let arrayGraduadosJavaScript = graduadosJavaScript.split(" ").map(Number);
let arrayGraduadosNodeJS = graduadosNodeJS.split(" ").map(Number);
...
function calcularPromedio(curso) {
  ...
  switch (curso) {
    case 1:
      cursoSeleccionado = arrayGraduadosHTML5;
      break;
    case 2:
      cursoSeleccionado = arrayGraduadosCSS3;
      break;
    case 3:
      cursoSeleccionado = arrayGraduadosJavaScript;
      break;
    case 4:
      cursoSeleccionado = arrayGraduadosNodeJS;
      break;
    default:
      return "Los cursos disponibles son:\n1 = HTML5\n2 = CSS3\n3 = JAVASCRIPT\n4 = NODEJS";
  }


  let suma = cursoSeleccionado.reduce(function (a, b) {
    return a + b;
  }, 0);

  let promedio = suma / cursoSeleccionado.length;
