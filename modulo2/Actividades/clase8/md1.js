...
let cursosAlumno = ["HTML", "JAVASCRIPT", "REACT"];

function calcularMonto(cursosDisp, cursosAlu){
let montoTotal = 0;

let cursosAlumnoMinusculas = cursosAlu.map(curso => curso.toLowerCase())

for(let i = 0;i<cursosAlumnoMinusculas.length; i++){
  let cursoSeleccionado = cursosAlumnoMinusculas[i];
  for (let j = 0; j < cursosDisp.length; j++){
    let curso = cursosDisp[j];
    if(curso[0] === cursoSeleccionado){
      montoTotal += curso[1];
    }
...
function generarInforme (nombre, apellido, cursosDispo, cursosAlum){
  let montoTotal = calcularMonto(cursosDispo, cursosAlum);

  let informe = `Estimado ${nombre} ${apellido}, en función de los cursos seleccionados:\n`;
  for (let i = 0 ; i < cursosAlum.length; i++){
    informe += `El monto total a pagar es de: $${montoTotal}.\nBienvenido a la gran aventura Digital House.`;

    return informe;
  }
  
}
console.log(generarInforme("Luis", "Fuentes", cursosDisponibles, cursosAlumno));
